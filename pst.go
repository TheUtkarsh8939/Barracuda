...
func setPhaseTables(tables *PST, phase int, whiteA8 [7][64]int) {
	for pt := chess.King; pt <= chess.Pawn; pt++ {
		...
		tables[phase][chess.Black][pt] = mirrorBoard(white)
	}
}
...
func initPST() PST {
	...
	startWhite[chess.Knight] = [64]int{
	...
	startWhite[chess.Bishop] = [64]int{
	...
	// Middlegame PST (PeSTO mg tables, A8..H1 orientation).
	midWhite := [7][64]int{}
	midWhite[chess.Pawn] = [64]int{
		0, 0, 0, 0, 0, 0, 0, 0,
	...
	// Endgame PST (PeSTO eg tables, A8..H1 orientation).
	endWhite := [7][64]int{}
	endWhite[chess.Pawn] = [64]int{
		0, 0, 0, 0, 0, 0, 0, 0,
	...
	setPhaseTables(&tables, pstStart, startWhite)
	setPhaseTables(&tables, pstMiddle, midWhite)
	setPhaseTables(&tables, pstEnd, endWhite)
